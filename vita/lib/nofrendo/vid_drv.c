...
#ifdef TARGET_VITA
// static allocation to avoid heap fragmentation on Vita (128MB limit)
static uint8_t _pb_data[(256 + 3) * 240 + 3]; // width*height + alignment
static struct { bitmap_t bmp; uint8 *lines[240]; } _pb_store;
...
{
#ifdef TARGET_VITA
   // use static storage — no malloc, no free, no heap fragmentation
   primary_buffer = &_pb_store.bmp;
   primary_buffer->hardware = false;
   primary_buffer->width = width;
   primary_buffer->height = height;
   primary_buffer->pitch = (width + 3) & ~3;
   primary_buffer->data = _pb_data;
   primary_buffer->line[0] = (uint8 *)(((uint32)_pb_data + 3) & ~3);
   { int i; for (i = 1; i < height; i++)
      primary_buffer->line[i] = primary_buffer->line[0] + i * primary_buffer->pitch; }
#else
...
#ifdef TARGET_VITA
   // static allocation — just NULL the pointer, nothing to free
   primary_buffer = NULL;
#else
   if (NULL != primary_buffer)
      bmp_destroy(&primary_buffer);
